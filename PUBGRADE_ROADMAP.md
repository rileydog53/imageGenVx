...
|---|---|---|---|
| 1 | **sizing** | ✅ | Content-aware molecule sizing landed (keystone `131a918`): every structure renders at one consistent bond length, no hand-set `scale`. **Glyph/blob sizing closed 2026-06-26:** GLYPH/BLOB slots no longer fill the uniform 180×140 cell — a GLYPH draws at its primitive's registered bbox (`tablet` 40, `pg_cluster` 50, `protein_blob` 96), a BLOB at a dedicated generous container box (`tier_blob_size`, sized to hold a ~120px substrate). Fixes fig 07 (ATP/ADP tablets dwarfed the substrate). The corpus's per-glyph `scale` compensation hacks (0.5–0.6) were removed (scale=1.0 natural is now proportionate). +5 tests. Suite 1224 → 1229. |
